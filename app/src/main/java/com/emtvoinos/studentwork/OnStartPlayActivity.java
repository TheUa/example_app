...
public class OnStartPlayActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ArrayList<Wheel> slots = stMachine.getSlots();
        wheel1 = slots.get(0);
        wheel2 = slots.get(1);
        wheel3 = slots.get(2);
        ...
        stMachine.setPlayerFunds(startMoney);
        Integer playerMoney = stMachine.checkPlayerFunds();
    ...
    public void updatePlayerMoney(){
        Integer newPlayerFunds = stMachine.checkPlayerFunds();
        playerFunds.setText(newPlayerFunds.toString());
    }

    public boolean checkPlayerBust(){
        int money = stMachine.checkPlayerFunds();
        return money <= 0;
    }

    public void onCollectButtonClicked(View button){
        Integer endMoney = stMachine.checkPlayerFunds();
    ...
    public void onAddMoneyClicked(View button){
        ...
        i.putExtra("currentFunds", stMachine.checkPlayerFunds());
        startActivity(i);
    }
    ...
    public void gameOver(){
        ...
        if (random == 1) {
            Intent i = new Intent(this, BonusMiniGameActivity.class);
            startActivity(i);
        }
        else {
    ...
    public ArrayList<Symbols> spin(){
        final ArrayList<Symbols> newLine = stMachine.spin();
        final int value = stMachine.getWinValue(newLine);
        final Handler handler = new Handler();
        handler.postDelayed(new Runnable() {
            public void run() {
    ...
    public void updateCurrentLine(ArrayList<Symbols> newLine){
        ArrayList<String> lineImages = stMachine.getLineImages(newLine);
    ...
    public void updateTopLine(){
        ArrayList<Symbols> topLine = stMachine.getTopLineSymbols();
        ArrayList<String> lineImages = stMachine.getLineImages(topLine);
        String image1 = lineImages.get(0);
        String image2 = lineImages.get(1);
        String image3 = lineImages.get(2);
        int idImage1 = getResources().getIdentifier(image1, "drawable", getPackageName());
    ...
    public void updateBottomLine(){
        ArrayList<Symbols> bottomLine = stMachine.getBottomLineSymbols();
        ArrayList<String> lineImages = stMachine.getLineImages(bottomLine);
        String image1 = lineImages.get(0);
        String image2 = lineImages.get(1);
        String image3 = lineImages.get(2);
        int idImage1 = getResources().getIdentifier(image1, "drawable", getPackageName());
    ...
    public void checkNudgeWin(){
        ArrayList<Symbols> newLine = stMachine.getCurrentSymbols();
        final int value = stMachine.getWinValue(newLine);
        if(stMachine.checkWin(newLine)){
            hideAllNudgeButtons();
            win(value);
    ...
    public int getCurrentSymbolIdAfterNudge(int wheelIndex){
        ArrayList<Symbols> currentLine = stMachine.getCurrentSymbols();
        ArrayList<String> lineImages = stMachine.getLineImages(currentLine);
    ...
    public int getTopLineSymbolIDAfterNudge(int wheelIndex){
        ArrayList<Symbols> topLine = stMachine.getTopLineSymbols();
        ArrayList<String> topImages = stMachine.getLineImages(topLine);
    ...
    public int getBottomLineSymbolIdAfterNudge(int wheelIndex){
        ArrayList<Symbols> bottomLine = stMachine.getBottomLineSymbols();
        ArrayList<String> bottomImages = stMachine.getLineImages(bottomLine);
